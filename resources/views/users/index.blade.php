...
<div class="table-responsive"><table><thead><tr><th>Nombre</th><th>Correo</th><th>Rol</th><th>Estado</th><th></th></tr></thead><tbody>
@forelse($users as $user)
<tr><td>{{ $user->name }}</td><td>{{ $user->email }}</td><td>{{ $user->role }}</td><td>{{ $user->active ? 'Activo' : 'Inactivo' }}</td><td class="actions">@if($user->isProtectedAdmin())<span class="badge">Protegido</span>@else<a class="btn light" href="{{ route('users.edit', $user) }}">Editar</a><form method="post" action="{{ route('users.destroy', $user) }}">@csrf @method('delete')<button class="btn danger swal-confirm" data-title="Eliminar usuario?" data-text="Si tiene ventas, se desactivara en lugar de eliminarse." data-confirm="Si, continuar">Eliminar</button></form>@endif</td></tr>
