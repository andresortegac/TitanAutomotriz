...
<tr><td>@if($product->image_url)<img class="product-photo" src="{{ $product->image_url }}" alt="{{ $product->name }}">@else<div class="product-photo-empty">Sin foto</div>@endif</td><td>{{ $product->code }}</td><td>{{ $product->sku ?? 'Pendiente' }}</td><td>{{ $product->primaryBarcode->code ?? 'Pendiente' }}</td><td>{{ $product->name }}</td><td>{{ $product->category->name }}</td><td>@if($product->stock <= $product->min_stock)<span class="badge warn">{{ $product->stock }}</span>@else {{ $product->stock }} @endif</td><td>${{ number_format($product->sale_price, 0) }}</td><td>{{ number_format($product->tax_rate, 2) }}%</td><td class="actions">@if($product->primaryBarcode)<a class="btn secondary" target="_blank" rel="noopener" href="{{ route('products.barcode-label', $product) }}">Imprimir barras</a>@endif @if(auth()->user()->isAdmin())<a class="btn light" href="{{ route('products.edit', $product) }}">Editar</a><form method="post" action="{{ route('products.destroy', $product) }}">@csrf @method('delete')<button class="btn danger swal-confirm" data-title="Eliminar producto?" data-text="Solo se eliminara si no tiene ventas registradas." data-confirm="Si, eliminar">Eliminar</button></form>@elseif(! $product->primaryBarcode) <span class="muted">Sin código</span> @endif</td></tr>
